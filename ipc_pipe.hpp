...
#ifndef DMITIGR_OS_IPC_PIPE_HPP
#define DMITIGR_OS_IPC_PIPE_HPP
  ...
  const std::function<void(pid_t)>& progress_handler)
{
  if (!(poll_timeout >= std::chrono::milliseconds::zero()))
    throw Exception{"invalid polling timeout"};
